 ...
 * =====================================================
 */
var mui = function(a, b) {          //a:document;b:undefined;
        ...
        h = /matrix(3d)?\((.+?)\)/,
        i = function(b, c) {
                if (c = c || a, !b) return j();   //[],[].selector='',[] instanceOf i.fn ==true;
                ...
                if ("string" == typeof b) try {
                        if (b = b.trim(), d.test(b)) {
                                var e = a.getElementById(RegExp.$1);  //RegExp.$1 ：第一个子匹配字符串;
                                return j(e ? [e] : [])
                        }
        ...
        j = function(a, b) {           //创建a对象的原型i.fn,并给a对象添加selector属性保存选择器b,若b不存在,则存放的选择器为空，最后返回a;
                 a = a || [];
                Object.setPrototypeOf(a, i.fn);    //将i.fn对象设置成a的原型;i.fn={each:function(){}};
                a.selector = b || "" ;
                return a
        };
        i.uuid = 0,
        ...
        i.each = function(a, b, c) {   //a是一个数组或类数组，b是一个函数;
                if (!a) return this;
        ...
        i.trigger = function(a, b, c) {   //a:对象;b:事件名;c是事件实例对象;
        ...
        i.hooks = {},
        i.addAction = function(a, b) {   //添加自定义行为；调用该函数返回对应行为名a的数组对象，其中存放的是定义的行为的相关值;
                var c = i.hooks[a];
                return c || (c = []),
                b.index = b.index || 1e3,
                c.push(b),
...
//mui.os对象存放浏览器信息；包括终端是android或是iphone,以及版本号以及isBadAndroid;{android:true,version:'5.0',isBadAndroid:false}或{iPhone:true,version:'5.0',isBadAndroid:false}
...
function(a) {
        ...
        g = function(b, c) {        //b为事件对象;          //对事件对象进行数据添加;添加了isDefaultPrevented、isImmediatePropagationStopped、isPropagationStopped属性方法;以及detail属性;
                 b.detail ? b.detail.currentTarget = c: b.detail = {
                        currentTarget: c
                },
                a.each(d,
                function(a, c) {   //值，属性;  
                        var d = b[a];
                        b[a] = function() {
                                return this[c] = e,//b.isDefaultPrevented=function(){return !0}
        ...
        h = function(a) {
                return a && (a._mid || (a._mid = b++))  //返回一个a对象的_mid编号;
        },
        i = {},
        j = function(b, d, e, f) {
                return function(e) {
...
function(a, b, c) {//mui,window,document;
        a.targets = {},
        a.targetHandles = [],
        a.registerTarget = function(b) {     //该函数传参一个模块对象，并将模块对象存放到targetHandles数组中，并按index进行排序,返回的是新的targetHandles模块对象的集合;
        ...
        function(b) {
                for (var d = b.target,
                e = {}; d && d !== c; d = d.parentNode) {  //遍历目标对象及冒泡经过的元素;
                        var f = !1;
						/*
						  b.targetHandles:{
							  {
								  handle:fn,index:40,name:'click',target:false
							  },
							  {
								  handle:fn,index:50,name:'action',target:false
							  },
							  {
								  handle:fn,index:50,isContinue:true,isReset:!1,name:'modal',target:false
							  },
							  {
                         		  handle:fn,index:50,isContinue:true,isReset:!1,name:'offcanvas',target:false
							  },
							  {
								  handle:fn,index:60,isContinue:true,isReset:!1,name:'popover',target:false
							  },
							  {
								  handle:fn,index:80,name:'tab',target:fale
							  },
							  {
								  handle:fn:index:100,name:'toggle',target:false
							  }
						...
						*/
                        ...
                        function(c, g) {
                                var h = g.name; //注册的模块名；如:offcanvas;注册的模块handle方法要么返回特定的DOM节点对象，要么返回false;
 							   ...
 							   /*
								   如:点击checkbox的input事件源;
								   1:a.target={click:input节点}, e={click:true} ,g.isContinue=undefined,f=true;
								   2:e={action:undefined} a.target={click:input节点,action:false}
								   3:e={modal:undefined} 
								   4:e={offcanvas:undefined} 
								   5:e={popover:undefined} 
								   6:e={tab:undefined} a.target={click:input节点,action:false,tab:false}
								   7:e={toggle:undefined} a.target={click:input节点,action:false,tab:false,toggle:false}
								*/        
					  }), f) break  
                }
        ...
        function(b) {
				...
				//若事件源是a节点，则去进行targetHandles处理，不是a节点，就遍历其父元素,并做父元素响应的处理;
                function(a, c) {
                        c.name;
		...
		/*
		     事件传播过程中，一旦某个事件源进行了targetHandles事件处理，则阻止其默认操作，并且targetHandles中后续的处理就不再执行，并继续传播至上一层做类似处理;
		*/
} (mui, window, document),
function(a) {
...
function() {
		...
		//console.log(window.CustomEvent);
} (),
...
function(a, b, c) {
	//mui,window,'click'    
        if ((a.os.android || a.os.ios) && !b.FastClick) {
				...
				/*
				     d函数处理DOM对象，属于表单类input对象，若果非input类型或者非单选、复选框又或者未激活则返回false,否则返回b.
				*/
                a.registerTarget({
                        name: c,
                        index: 40,
...
function(a, b) {    //mui,window
        ...
        t = function(b) {
				...
				*/
                ...
                n = b.center = p(d);
				/*
				    c添加了center属性;指代手指的平均所在位置;
				*/
                b.timestamp = a.now(),
				/*
				    c添加了timestamp属性，当前时间戳;
				*/
        ...
        y = function(b, c) {
				...
				/*
				  d:存放事件对象的集合，并给予其集合的一个属性identifier为0;
				*/
                e = b.type,//事件类型;
                f = [],
                g = [];
                if (e !== a.EVENT_START && e !== a.EVENT_MOVE || 1 !== d.length) {
					/*
					    d.length不为1指的是在移动端多个手指触发的事件;
						e类型既不是EVENT_START也不是EVENT_MOVE事件;
		                PC端非EVENT_START/EVENT_MOVE不能执行这块代码;
					*/
                        var h = 0,
                        f = [],
                        g = [],
                ...
                } else w[d[0].identifier] = !0,//让W对象为:{0:true};
                f = d,
                g = d,
                c.target = b.target;
                f = o(f.concat(g), "identifier", !0);
				/*
				    identifier是事件对象的标识，若标识了这会加入新的队列，全部没有标识的话，则取第一个事件对象。而第一个事件对象必定在新的集合中;而且新的集合中排序是按identifier来排序;
				*/
                var k = f.length,//进过筛选后集合的长度;
                l = g.length;//原来集合的长度;
                ...
                c.touches = f,//touches存放的筛选后的对象事件;
                c.changedTouches = g,//changedTouches存放的最初的事件对象，带着identifier=0的标识;
                !0
				/*
				    c中添加了target/isFinal/touches/changedTouches属性;
					若为EVENT_START事件，并且筛选后的事件对象集合为1的话：
					           c会添加isFisrt属性，并赋值为true;
							   mui.gestures会添加一个touch属性，其值为:{target:b.target};
							   mui.gestures的session属性值为:{target:b.target};
					经过y(b,c)调用，c就变成了：
				                 	{
						                 gesture:b,
										 target:b.target,
										 isFinal:true/false,
										 touches:事件对象集(筛选后的),
										 changedTouches:最初的事件对象集合,
										 可能有的属性：
										 isFisrt:true/false
					                }
					经过y(b,c)调用，mui.gesture变成了：
										{
...
function(a) {
        var b = false,
        c = /xyz/.test(function() {xyz}) ? /\b_super\b/: /.*/,  //   返回正则：/\b_super\b/;
        d = function() {};
        d.extend = function(a) {
                function d() { 
...
function(a, b, c) {
        ...
        p = "mui-hidden",
        q = "mui-visibility",
        r = g + " " + l + " " + n,   //'mui-pull-loading mui-icon mui-icon-pulldown'
        s = g + " " + l + " " + n,   //'mui-pull-loading mui-icon mui-icon-pulldown'
        t = g + " " + l + " " + m,   //'mui-pull-loading mui-icon mui-spinner'
...
function(a, b, c, d) {   //mui, window, document, "offcanvas"
        var e = "mui-off-canvas-left",  //左滑块部分;
        f = "mui-off-canvas-right",  //右滑块部分;
        g = "mui-off-canvas-backdrop",  
        h = "mui-off-canvas-wrap", //最大的DIV盒子;
        i = "mui-slide-in",
        j = "mui-active",
        k = "mui-transitioning",
        l = ".mui-inner-wrap",    //内容页面部分;
        m = a.Class.extend({    //a.Class=function (){};
                init: function(b, d) {
                        this.wrapper = this.element = b,      
                        this.scroller = this.wrapper.querySelector(l),  //内容页面;
                ...
                refresh: function(a) {
                        this.slideIn = this.classList.contains(i),//i:"mui-slide-in"
                        this.scalable = this.classList.contains("mui-scalable") && !this.slideIn,
                        this.scroller = this.wrapper.querySelector(l), //".mui-inner-wrap"
                        this.offCanvasLefts = this.wrapper.querySelectorAll("." + e),//e: "mui-off-canvas-left"
                        this.offCanvasRights = this.wrapper.querySelectorAll("." + f),//f:"mui-off-canvas-right"
                        ...
                        this.backdrop = this.scroller.querySelector("." + g),//g:"mui-off-canvas-backdrop";
        ...
        a.fn.offCanvas = function(b) {
                ...
                return this.each(function() {
                        var d = null,
                        e = this;
                        e.classList.contains(h) || (e = n(e));   //h = "mui-off-canvas-wrap", //最大的DIV盒子;
...
function(a, b, c) {    //参数分别为:mui,window,'toggle';
        var d = "mui-switch",
        e = "mui-switch-handle",
        f = "mui-active",
        g = "mui-dragging",
        h = "mui-disabled",
        i = "." + e,   //i='.mui-switch-handle';
        j = function(a, b) {
