                ...
                if(Math.abs(E)>50 && (Math.abs(E)>Math.abs(D))){
                  if(E<0 && R!=l-1){					  
                     ...
                     // w.classList.remove('arrow-1');
					  //setTimeout(function(){w.classList.add('arrow-2');},100)					  
					  d.removeEventListener(J,Z,!1);					  
				  }else if(E> 0&& R!=0){
					  R--;
					  W(-R*M,S);
					 // w.classList.remove('arrow-2');
					//  setTimeout(function(){w.classList.add('arrow-1');},100)
					  d.removeEventListener(J,Z,!1)
				  }
				}
               				
			}
			 R != l -1 ? (w.classList.remove('arrow-2'),w.classList.add('arrow-1')) : (w.classList.remove('arrow-1'),w.classList.add('arrow-2'))
			a.preventDefault();
		},
		$=function(a){
			d.removeEventListener(J,Z,!1);
